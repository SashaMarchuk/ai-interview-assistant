...
must_haves:
  truths:
    ...
    - "webext-zustand message filter (chromex.dispatch, chromex.fetch_state) is the first check in onMessage listener, positioned before the queue guard check"
  ...
  key_links:
    ...
    - from: "entrypoints/background.ts"
      ...
      via: "import useStore and call useStore.getState().apiKeys.elevenLabs"
      pattern: "import.*useStore.*from.*src/store"
    - from: "entrypoints/background.ts"
      to: "offscreen"
    ...
    - from: "entrypoints/background.ts"
      to: "entrypoints/background.ts"
      via: "webext-zustand filter positioned BEFORE queue guard in onMessage listener"
      pattern: "chromex\\.dispatch.*chromex\\.fetch_state"
---
...
2. Confirm `StartTranscriptionMessage` no longer has `apiKey`: `grep -n 'apiKey' src/types/messages.ts` -- should only appear in `InternalStartTranscriptionMessage`, not in `StartTranscriptionMessage`.
3. Confirm `InternalStartTranscriptionMessage` exists with `apiKey` and `_fromBackground: true`: `grep -A5 'InternalStartTranscriptionMessage' src/types/messages.ts`
4. **Verify apiKey removed from handleStartTranscription sendMessage call:** Run `grep -n 'apiKey' entrypoints/popup/App.tsx | grep -v 'apiKeys'` -- confirm no `apiKey` match within the `handleStartTranscription` function's `sendMessage` call. Only matches should be outside `handleStartTranscription` (e.g., store selectors using `apiKeys`).
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: Add queue guard infrastructure for store hydration</name>
  <files>entrypoints/background.ts</files>
  <action>
**Queue guard for store hydration (REL-01)**
...
**Critical constraint:** The webext-zustand filter (step 1 in listener) MUST remain first. Queuing these messages causes deadlock because the store itself depends on these messages to hydrate.
  </action>
  <verify>
1. Run `npx tsc --noEmit` -- no type errors.
2. Verify `storeReady` flag and `messageQueue` exist in background.ts: `grep -n 'storeReady\|messageQueue' entrypoints/background.ts`
3. Verify queue guard check is present in the listener, AFTER the three early-return filters: `grep -n 'storeReady' entrypoints/background.ts`
4. **Wiring verification -- webext-zustand bypass position (line-number ordering check):** Run `grep -n 'chromex\.\(dispatch\|fetch_state\)\|!storeReady' entrypoints/background.ts`. Extract line numbers. Confirm that BOTH `chromex.dispatch` and `chromex.fetch_state` filter lines appear at STRICTLY LOWER line numbers than the `!storeReady` queue guard line. Example valid output: lines 440,440 for chromex filter vs line 455 for `!storeReady` (440 < 455 = correct). If the `!storeReady` line number is less than or equal to any chromex filter line number, it is a deadlock bug -- fix by moving the webext-zustand filter above the queue guard.
  </verify>
  <done>
Queue guard queues messages until store hydrates, then drains. 10-second timeout prevents infinite hang. webext-zustand messages (`chromex.dispatch`, `chromex.fetch_state`) bypass the queue by being filtered BEFORE the queue guard check. Build passes. No type errors.
  </done>
</task>

<task type="auto">
  <name>Task 3: Read ElevenLabs API key from store instead of message</name>
  <files>entrypoints/background.ts</files>
  <action>
**Read ElevenLabs API key from store (SEC-01)**

1. **Verify useStore import exists.** Check that `entrypoints/background.ts` already imports `useStore` from `src/store`. Run: `grep -n 'import.*useStore.*from' entrypoints/background.ts`. If the import does NOT exist, add it:
```typescript
import { useStore } from '@/store';
```
If it already exists (likely as part of existing webext-zustand wiring), confirm the import path resolves to `src/store/index.ts`.

2. Import `InternalStartTranscriptionMessage` from messages.ts at the top of background.ts (add to existing import from `@/types/messages`).

3. Modify the `START_TRANSCRIPTION` case in `handleMessage` (lines 699-725) to read the API key from the store instead of `message.apiKey`:
...
3. **Verify useStore import:** `grep -n 'import.*useStore' entrypoints/background.ts` -- must show an import line from `src/store` or `@/store`.
4. Verify `handleMessage` for `START_TRANSCRIPTION` reads `useStore.getState().apiKeys.elevenLabs`: `grep -n 'useStore.getState' entrypoints/background.ts`
5. Grep: `grep -n 'apiKey' entrypoints/popup/App.tsx` should NOT match any line in `handleStartTranscription`.
6. Grep: `grep -n 'message\.apiKey' entrypoints/background.ts` should return zero matches (background no longer reads apiKey from message).
7. Grep: `grep -n 'InternalStartTranscriptionMessage' entrypoints/background.ts` -- confirms the import exists.
  </verify>
  <done>
Background reads ElevenLabs key from `useStore.getState().apiKeys.elevenLabs` (import verified). Internal forward to offscreen uses `InternalStartTranscriptionMessage` with `apiKey` from store. `message.apiKey` is never read in background.ts. Build passes. No type errors.
  </done>
</task>
