...
4. **Wiring verification -- webext-zustand bypass position:** Run `grep -n 'chromex\.\(dispatch\|fetch_state\)\|storeReady' entrypoints/background.ts` and confirm that the `chromex.dispatch` / `chromex.fetch_state` filter lines appear at LOWER line numbers than the `!storeReady` queue guard check. If the queue guard appears first, it is a deadlock bug -- fix by moving the webext-zustand filter above the queue guard.
  </verify>
  <done>
